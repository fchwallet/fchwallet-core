...
public class BrdApiClient {
    ...
    void sendJsonRequest(String networkName,
        ...
        makeAndSendRequest(Arrays.asList("ethq", getNetworkName(networkName), "proxy"), ImmutableMultimap.of(), json, "POST",
                new BrdResponseHandler<>(handler));
    }
    ...
    private <T> void makeAndSendRequest(List<String> pathSegments,
        ...
        if (json == null) {
            ...
            httpBody = RequestBody.create(MAPPER_JSON.writeValueAsString(json), MEDIA_TYPE_JSON);
    ...
    private static class BrdResponseHandler<T> implements ResponseHandler {
        ...
        BrdResponseHandler(CompletionHandler<T, QueryError> handler) {
            TypeFactory typeFactory = MAPPER_JSON.getTypeFactory();

            this.type = typeFactory.constructParametricType(BrdResponse.class, String.class);
            this.handler = handler;
        }

        @Override
        public void handleResponse(String responseData) throws JsonProcessingException {
            BrdResponse<T> resp = MAPPER_JSON.readValue(responseData, type);
            if (resp == null) {
    ...
    private static class BrdResponseWithStatusHandler<T> implements ResponseHandler {
        ...
        BrdResponseWithStatusHandler(Class<T> clazz, CompletionHandler<List<T>, QueryError> handler) {
            ...
            this.type = typeFactory.constructParametricType(BrdResponseWithStatus.class, inner);
            this.handler = handler;
        }

        @Override
        public void handleResponse(String responseData) throws JsonProcessingException {
            BrdResponseWithStatus<List<T>> resp = MAPPER_JSON.readValue(responseData, type);
            if (resp == null) {
                QueryError e = new QueryModelError("Transform error");
                Log.log(Level.SEVERE, "parsing error", e);
                handler.handleError(e);
    ...
    private static class ListResponseHandler<T> implements ResponseHandler {
        ...
        ListResponseHandler(Class<T> clazz, CompletionHandler<List<T>, QueryError> handler) {
            ...
            this.type = typeFactory.constructCollectionLikeType(ArrayList.class, clazz);
            this.handler = handler;
        }

        @Override
        public void handleResponse(String responseData) throws JsonProcessingException {
            List<T> data = MAPPER_JSON.readValue(responseData, type);
            if (null == data) {
                QueryError e = new QueryModelError("Transform error");
                Log.log(Level.SEVERE, "parsing error", e);
                handler.handleError(e);
